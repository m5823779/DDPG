...
class ActorNetwork:
	...
	def create_network(self,state_dim,action_dim):
		...
		W5 = tf.Variable(tf.random_uniform([layer3_size,1],-3e-3,3e-3))
		...
		action_linear = tf.sigmoid(tf.matmul(layer3_bn,W4) + b4)
		action_angular = tf.tanh(tf.matmul(layer3_bn, W5) + b5)
		action_output = tf.concat([action_linear, action_angular], 1)

		return state_input,action_output,[W1,b1,W2,b2,W3,b3,W4,b4,W5,b5],is_training
	...
	def create_target_network(self,state_dim,action_dim,net):
		...
		action_linear = tf.sigmoid(tf.matmul(layer3_bn,target_net[6]) + target_net[7])
		action_angular = tf.tanh(tf.matmul(layer3_bn, target_net[8]) + target_net[9])
		action_output = tf.concat([action_linear, action_angular], 1)
	...
	def train(self,q_gradient_batch,state_batch):
		...
		if Plot:
			vis.line(X=np.array([self.time_step]), Y=np.array([actor_loss]), win=grad, update='append', name='actor_grad')
